/**
 * @file legacy_main.cpp
 * @brief Legacy main program using old HeatUtils API with new architecture backend
 *
 * This program demonstrates the compatibility layer by using the old API
 * while the implementation uses the new architecture classes internally.
 *
 * This maintains backward compatibility while transitioning to the new architecture.
 *
 * @note This file generates deprecation warnings during compilation.
...
/**
 * 2D Heat Equation Solver using MPI Parallel Computing (Legacy Version)
...
int main(int argc, char** argv) {
    ...
    if (numProcs != Npx * Npy) {
        if (myRank == 0) {
            std::cerr << "Error: Required " << Npx * Npy << " processes, but got "
    ...
    int myCoord[nDims];
    MPI_Cart_coords(SBD_COMM, cartRank, nDims, myCoord);
    ...
    for (int nStep = 0; nStep < Nnt; nStep++) {
        ...
        // Note: Using the legacy MPIJacobi function
        MPIJacobi(Sol, x0, b, Residu, Tol, iConv, Nx, Ny, lambda,
                  NeighbourRank, SBD_COMM, colType, myRank);

        // Display iteration info (only from root process)
        if (myRank == 0) {
            std::cout << "Step=" << nStep << " Residu=" << Residu
                      << " Iter=" << iConv << std::endl;
        }
    }
    ...
    if (myRank == 0) {
        std::cout << "Numerical Approximation: [Euler Implicit] (Legacy Version with New Architecture)" << std::endl;
        ...
        std::string ThisFile = "ExactImp-legacy.txt";
        Export(Ref, I, J, x, y, Nx, Ny, ThisFile);
        ThisFile = "SolImp-legacy.txt";
        Export(Sol, I, J, x, y, Nx, Ny, ThisFile);
    }

    // Free memory (legacy style)
    delete[] Sol[0];
    delete[] Sol;
    delete[] Sol0[0];
    delete[] Sol0;
